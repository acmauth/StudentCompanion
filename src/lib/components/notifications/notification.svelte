<script lang="ts">
	...
	const inlineModalDismissed = (val: any) => {
		inlineModalOpen = false;
	};

	let content = DOMPurify.sanitize(notification.body, { SANITIZE_NAMED_PROPS: true })
		.trim()
		.replace(/\s+/g, ' ')
		.replace(/\s+/g, ' ');
...
>
	<div class="top">
		<img
			alt="Service logo"
			src={notification.type == 'universis'
				? universisLogo
				: notification.type == 'webmail'
				? mail
				: elearningLogo}
		/>
			...
			<ion-item-group>
				...
				<div class="context-chips ion-padding-horizontal">
					<ion-chip
						style="flex-shrink: 2;"
						outline
						color={notification.type == 'universis'
							? 'tertiary'
							: notification.type == 'webmail'
							? 'secondary'
							: 'warning'}
				...
				<ion-item-divider>
					<ion-label
						>{$t('notifications.date')}: {notification.dateReceived.toLocaleString()}</ion-label
					>
				</ion-item-divider>
			</ion-item-group>
		</div>
