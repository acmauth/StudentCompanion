<script>
	...
	let targetNumber = 0;
	const maxIterations = course.formattedGrade;
	...
	const primaryColor = getComputedStyle(document.body)
		.getPropertyValue('--app-color-stat-graph')
		.trim();
	...
	afterUpdate(() => {
		if (course.formattedGrade !== undefined) {
			targetNumber = course.formattedGrade;
			startSlotMachine();
		}
	});
...
<ion-card>
	...
	<ion-card-content class="ion-text-center">
		...
		{:else if course.isPassed}
			<ion-text class="success" id="number">{course.formattedGrade}</ion-text>
		{:else}
			<ion-text class="danger" id="number">{course.formattedGrade}</ion-text>
		{/if}
...
<ion-card>
	...
	<ion-card-content>
		...
		{:else}
			<ion-list>
				<ion-item>
					<ion-text class="ion-padding" color="danger">{$t('course.no_stats')}</ion-text>
				</ion-item>
			</ion-list>
		{/if}
	</ion-card-content>
